...
exports.pic = function(opt, url, data){
  ...
  if(opt.prefs.stockageLocal){
    if(opt.prefs.localDirectory !== undefined){
      var writePath = OS.Path.join(opt.prefs.localDirectory, fileName);
    }else{
      let makeDir = OS.File.makeDir(OS.Path.join(OS.Constants.Path.desktopDir,'krayon'));
      var writePath = OS.Path.join(OS.Constants.Path.desktopDir,'krayon',fileName);
    }

    let promise = OS.File.writeAtomic(writePath, blobCanvas, {tmpPath:writePath + '.tmp'});
    promise.then(
      function success(){
        includeJS.port.emit("displayABBDconsole", 1);
        if(!opt.prefs.stockageremote){
          //Weird hack for xubuntu (at least) because tabs.open reinterpret badly local path
          tabs.open({url:"https://bugzilla.mozilla.org",onOpen: function(tab){
	  	      tab.url = writePath;
          }});
        }
      },
      function failure(because){
        console.log('failure writing file '+because);
      }
    );
  }
